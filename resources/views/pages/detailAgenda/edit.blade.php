<form
    ...
    <div class="form-group">
        ...
        <select name="agenda_id" class="form-control" id="">
            @foreach ($agenda as $d)
                <option value="{{ $d->id }}" @if($d->id == $data->agenda_id) selected @endif>{{ $d->judul }}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group">
        <label for="">Nama</label>
        <input value="{{ $data->judul }}" name="judul" type="text" class="form-control" placeholder="example..">
    </div>
    <div class="form-group">
        <label for="">start</label>
        <input value="{{ $data->start }}" name="start" type="date" class="form-control" placeholder="example..">
    </div>
    <div class="form-group">
        <label for="">end</label>
        <input value="{{ $data->end }}" name="end" type="date" class="form-control" placeholder="example..">
    </div>
    <div class="form-group">
...
<script>
        $('#createAgenda').submit(function(e) {
            ...
            $.ajax({
                type:'POST',
                url: '{{ route('detailagenda.update', $data->id) }}',
                data: formData,
                contentType: false,
                processData: false,
                success: (response) => {
